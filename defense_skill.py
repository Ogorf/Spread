...
class DefensePerk(skilltree.Perk):
...
class Preparation(DefensePerk):
    ...
    def defense_modifier(self, info):
        if self.skilled > 0:
            ...
            result = ((t - max(0 if attack_list == [] else attack_list[-1][0],
                               0 if defend_list == [] else defend_list[-1][0],
                               0 if conquer_list == [] else conquer_list[-1][0])) * self.values[self.skilled - 1][0]) / 1000
            return int(result)
        else:
            return 0
...
class DefenseSkill(skilltree.Skill):
    def __init__(self, l):
