...
class InfectionPerk(skilltree.Perk):
    pass
...
class Base(InfectionPerk):
    ...
    def get_value(self, info):
        if self.skilled > 0 and info["current_time"] - info["arrival_time"] <= info["bubble"].population / \
                self.values[self.skilled - 1][0]:
            return 1
        else:
            return 0


class InfectionSkill(skilltree.Skill):
    def __init__(self, l):
