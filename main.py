...
def redraw_game_window(screen, selected):
    font = pygame.font.SysFont("comincsans", 20)
    ...
    for obj in Cell._registry:
        obj.draw(screen)
        poptext = font.render(str(obj.population), 1, (0, 0, 0))
        screen.blit(poptext, (obj.xcord - 3, obj.ycord - 5))
...
def adjust_cell(cell):
    ...
    if not enough_space:
        ...
        messageboxwindow = Tk()
        messageboxwindow.eval("tk::PlaceWindow %s center" % messageboxwindow.winfo_toplevel())
        messageboxwindow.withdraw()

        messagebox.showerror("Can not execute orders", "Cell is to close to another cell.")

        messageboxwindow.destroy()
        messageboxwindow.quit()
    ...
    while running_editor:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if EditorButton._registry and pygame.Rect(EditorButton._registry[0].rect).collidepoint(pos):
                    adjust_cell(selected[0])
                    selected.clear()
                    EditorButton._registry[0].effect()

                for box in TextBox._registry:
                    if pygame.Rect(box.rect).collidepoint(pos):
                        box.active = True
                        box.text = ""
                    else:
                        box.active = False
